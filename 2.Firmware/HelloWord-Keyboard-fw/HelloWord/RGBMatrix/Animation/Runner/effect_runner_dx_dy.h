...
#ifndef HELLOWORD_KEYBOARD_FW_EFFECT_RUNNER_DX_DY_H
#define HELLOWORD_KEYBOARD_FW_EFFECT_RUNNER_DX_DY_H
...
bool effect_runner_dx_dy(effect_params_t* params, dx_dy_f effect_func) {
  ...
  for (uint8_t i = led_min; i < led_max; i++) {
    ...
    RGB     rgb = hsv_to_rgb(effect_func(g_rgb_matrix_config.rgb_config.hsv, dx, dy, time));
    rgb_matrix_set_color_with_rgb(i, rgb);
  }
  return rgb_matrix_check_finished_leds(led_max);
}

#endif // HELLOWORD_KEYBOARD_FW_EFFECT_RUNNER_DX_DY_H
